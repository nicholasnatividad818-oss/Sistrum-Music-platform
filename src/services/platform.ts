...
export async function loadPlatformData(user: User | null): Promise<PlatformData> {
  const baseQueries = await Promise.all([
    ...
    supabase.from('playlists').select('*').order('created_at', { ascending: false }).limit(200),
    supabase.from('playlist_tracks').select('*').order('position', { ascending: true }).limit(1000),
  ]);
...
export async function publishTrack(input: PublishTrackInput): Promise<void> {
  ...
  if (input.coverFile && (!input.coverFile.size || input.coverFile.size > 10 * 1024 * 1024)) throw new Error('Choose cover art under 10 MB.');
  if (!input.coverFile && input.coverPresetUrl && !input.coverPresetUrl.startsWith('https://')) throw new Error('Cover art must be uploaded or use an HTTPS URL.');
  ...
  try {
    ...
    const cleanup = await Promise.allSettled(uploaded.map((item) => supabase.storage.from(item.bucket).remove([item.path])));
    if (cleanup.some((result) => result.status === 'rejected' || result.value.error)) {
      throw new Error('Publishing failed and uploaded files could not be cleaned up. Contact support before uploading again.', { cause: error });
    }
    throw error;
  }
}
