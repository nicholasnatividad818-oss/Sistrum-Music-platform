...
using (
  exists (
    ...
    select 1 from public.tracks t where t.id = track_id
  )
);
...
drop policy if exists "Owners read own upload metadata" on storage.objects;

create policy "Owners read own upload metadata" on storage.objects
for select to authenticated
using (
  bucket_id in ('audio', 'cover-art') and
